...
function plus_setting_section_callback_function() {
	echo '<p>Simply add a <a href="https://plus.google.com/">Google+</a> account ID to generate a <a href="http://en.wikipedia.org/wiki/Favicon">favicon</a> for your blog, feed logo and admin logo.<br />Enter your G+ account ID</p>';
}
 
function plus_setting_callback_function() {
	if ( get_option('plus_id') && get_plus_photo() ) {
		$site_icon = str_replace( 'photo.jpg', 's16-c/photo.jpg', get_plus_photo() );
		$site_icon = ' <span><img src="'. $site_icon .'" /></span>';
	}

	echo '<input name="plus_id" id="plus_id" type="text" value="'. get_option('plus_id') .'" class="regular-text" />' . $site_icon;
}
...
function get_plus_photo(){
	...
	if ( !$gson = wp_cache_get( 'gplus-fav', 'general' ) ) {
		...
		wp_cache_add( 'gplus-fav', $gson, 'widget', 86400); //cache for 24 hours
	...
	if ( 'error' != $gson ) :
		...
		if ( $plus ){
			...
			$image_from_plus = str_replace( 'photo.jpg', 's20-c/photo.jpg', $plus );
